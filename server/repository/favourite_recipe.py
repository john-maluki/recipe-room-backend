...
class FavouriteRecipeRepository:
    def get_favourite_recipe_by_user_and_recipe(
        user_id: int, recipe_id: int, db: Session
    ):
        return (
            db.query(Favourite).filter_by(user_id=user_id, recipe_id=recipe_id).first()
        )

    def create_or_delete_favourite_recipe(user_id: int, recipe_id: int, db: Session):
        favorite = (
            db.query(Favourite).filter_by(user_id=user_id, recipe_id=recipe_id).first()
        )
        if favorite:
            db.delete(favorite)
            db.commit()
            return {"id": favorite.id, "user_id": user_id, "recipe_id": recipe_id}
        else:
    ...
    def get_favourite_recipes_by_user(
    ...
    def delete_favourite_recipe(db: Session, favourite_id: int):
        favourite_recipe = (
            db.query(Favourite).filter(Favourite.id == favourite_id).first()
        )
        if favourite_recipe:
