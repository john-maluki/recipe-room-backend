...
from ..schemas import FavouriteRecipe, RecipeSchema, CreateFavouriteSchema
...
@router.post(
    "/favourite", response_model=FavouriteRecipe, dependencies=[Depends(JWTBearer())]
)
async def create_or_delete_favourite_recipe(
    data: CreateFavouriteSchema, db: Session = Depends(get_db)
):
    favorite = FavouriteRecipeRepository.create_or_delete_favourite_recipe(
        data.user_id, data.recipe_id, db
    )
    return favorite


@router.get(
    "/{user_id}", response_model=List[RecipeSchema], dependencies=[Depends(JWTBearer())]
)
...
@router.get(
    "/{favourite_id}",
    response_model=FavouriteRecipe,
...
async def get_favourite_recipe_by_id(favourite_id: int, db: Session = Depends(get_db)):
    ...
    favourite_recipe = FavouriteRecipeRepository.get_favourite_recipe_by_id(
        db, favourite_id
    )
...
@router.delete(
    "/{favourite_id}", response_model=dict, dependencies=[Depends(JWTBearer())]
)
async def delete_favourite_recipe(
    favourite_id: int,
    user_id: int = Depends(get_user_id),
    db: Session = Depends(get_db),
):
    existing_favourite = FavouriteRecipeRepository.get_favourite_recipe_by_id(
        db, favourite_id
    )
