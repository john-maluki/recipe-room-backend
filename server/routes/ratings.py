...
@router.post(
    "/",
    response_model=ShowRatingSchema,
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(JWTBearer())],
)
async def create_rating(
    rating_data: CreateRatingSchema,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_user_id),
):
    existing_rating = RatingRepository.get_rating_by_user_and_recipe(
        db, user_id=current_user, recipe_id=rating_data.recipe_id
    )

    if existing_rating:
        updated_rating = RatingRepository.update_rating(
            db, existing_rating.id, rating_data
        )
...
@router.get(
    "/recipe/{recipe_id}",
    response_model=List[ShowRatingSchema],
    dependencies=[Depends(JWTBearer())],
)
async def get_ratings_by_recipe_id(recipe_id: int, db: Session = Depends(get_db)):
    ...
    if not ratings:
        raise HTTPException(
            ...
            detail="No ratings found for the specified recipe",
        )
...
async def get_ratings_by_user_id(user_id: int, db: Session = Depends(get_db)):
    ...
    if not ratings:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No ratings found for the specified user",
        )
...
@router.put(
    "/{rating_id}",
    response_model=ShowUpdatedRatingSchema,
    dependencies=[Depends(JWTBearer())],
)
async def update_rating(
    rating_id: int, rating_data: UpdateRatingSchema, db: Session = Depends(get_db)
):
...
async def delete_rating(
    ...
    if existing_rating.user_id != user_id_from_token:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You cannot to delete this rating",
        )
...
async def get_recipe_rating_for_user(
    recipe_id: int, user_id: int, db: Session = Depends(get_db)
):
    rating = RatingRepository.get_rating_by_user_and_recipe(db, user_id, recipe_id)
